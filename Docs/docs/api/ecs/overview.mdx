...
- **Fixed update stage** — no fixed-timestep accumulator or `FixedUpdate` schedule
- **Engine-owned render stage** — the render system currently lives in the demo, not the engine
- **System ordering API** — only raw flecs `ecs_dependson` phase ordering; no explicit before/after dependencies

See [Scheduler & Stages](../core/scheduler).
:::
