---
title: SafiMaterial
description: Graphics pipeline + sampler + base-color texture for unlit rendering.
---
...
:::tip Multi-material models
For glTF files with multiple materials, use [`SafiModel`](./gltf_loader) instead of managing `SafiMaterial` instances directly. `SafiModel` handles material deduplication, per-primitive texture binding, and fallback textures automatically.
:::
...
- [`SafiMesh`](./mesh)
- [`SafiModel`](./gltf_loader) — multi-material model loading
