...
Clear the handle (`.texture = {0}`) to fall back to the solid `color` again. The inspector's "Texture" field accepts a project-root-relative path and loads through the same asset registry.
