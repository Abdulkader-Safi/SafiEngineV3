...
Positioned on the **left side** of the window. Lists every entity that has a `SafiName` component. Clicking an entry selects it for the Inspector. The selected entity is highlighted with a distinct button color.
...
Vec3 properties (position, rotation, scale, color, direction) display as a label with three editable number cells in a single row. Number fields support **drag to scrub** and **double-click to type** a value directly.
