...
| `safi_editor_get_selected`  | Returns the entity currently shown in the Inspector.                                        |
| `safi_editor_set_selected`  | Sets the entity shown in the Inspector. Typically called once at startup to pick a default. |

:::info Selection source of truth
The selected entity is stored on `SafiEditorState.selected_entity`, not inside the debug UI. The Scene panel writes it on click; the Inspector, the [toolbar](../editor/editor_toolbar) highlight, and the [gizmo system](../editor/editor_gizmo) all read from the same singleton. Use `safi_editor_get_selected(world)` / `safi_editor_set_selected(world, e)` from any system; see [Editor State](../editor/editor_state).
:::
...
- [Editor State](../editor/editor_state) — where the selected entity is actually stored
- [Editor Toolbar](../editor/editor_toolbar) — Play/Pause/Stop + tool buttons drawn at the top of the viewport
